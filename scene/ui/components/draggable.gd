...
#region 静态变量
static var _saved_window_state: Dictionary = {}
#endregion
...
func set_content_min(v:Vector2)->void:
    ...
    # 撑大到内容所需尺寸(若已更大则不变),下限由 min_size 保证
...
func _on_drag_input(event: InputEvent) -> void:
    ...
    if event is InputEventMouseButton:
        ...
        # ...Motion 表示鼠标或笔的移动。如果鼠标正在拖动把手上移动且自己在被拖动中，就根据鼠标位置更新自己的位置
